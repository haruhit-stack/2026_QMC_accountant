...
function renderCart() {
  ...
  if (!currentCart.length) {
    ...
    currentCart.forEach(i => {
      ...
      r.querySelector(".cart-item-detail").textContent = `${yen(i.price)} × ${i.quantity} = ${yen(i.price * i.quantity)}`;
...
function formatDate(s) {
  const d = new Date(s);
  return Number.isNaN(d.getTime()) ? s : new Intl.DateTimeFormat("ja-JP", {
...
function renderHistory() {
  ...
  [...ts].reverse().forEach(t => {
    ...
    x.innerHTML = '<div class="history-top"><span class="amount"></span><span class="history-date"></span></div><div class="history-detail"></div>';
    x.querySelector(".amount").textContent = yen(t.amount);
    x.querySelector(".history-date").textContent = formatDate(t.timestamp);
    x.querySelector(".history-detail").textContent = `${(t.items || []).map(i => `${i.name} ×${i.quantity}`).join("、")}｜受取 ${yen(t.received)}｜お釣り ${yen(t.change)}`;
